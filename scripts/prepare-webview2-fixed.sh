...
if [[ "$MODE" == "nuget" && -z "$WEBVIEW2_CAB" && -z "$WEBVIEW2_URL" ]]; then
  if [[ -z "$NUGET_VERSION" ]]; then
    ...
    NUGET_INDEX="$(curl -fsSL --retry 3 --retry-delay 1 -A "adbchelper-build-script" https://api.nuget.org/v3-flatcontainer/webview2.runtime.x64/index.json || true)"
...
start "" "%APP_DIR%adbchelper.exe"
EOF2

echo "Fixed runtime extracted to: $RUNTIME_ROOT/$RUNTIME_DIR_NAME"
if [[ -f "$DIST_DIR/adbchelper.exe" ]]; then
  echo "Launcher created: $LAUNCHER"
else
  echo "Note: adbchelper.exe not found in $DIST_DIR yet. Build first, then re-run to refresh launcher." >&2
fi
